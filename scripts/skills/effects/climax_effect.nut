this.climax_effect <- this.inherit("scripts/skills/skill", {
	...
	function onTurnEnd()
	{
		...
		if (actor.getFlags().has("lewdOrgasmDefeat"))
		{
			...
			{
				local absorbed = ::Lewd.Mastery.absorbStat(killer, actor, false);
				if (absorbed != null)
				{
					this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(killer) + " absorbs +1 " + absorbed.label + " from " + this.Const.UI.getColorizedEntityName(actor));
					::Lewd.Mastery.playAbsorbEffect(killer, this.Tactical, this.Const, this.Sound, this.Time);
				}
			}
